    ...
    * CONTAINS:
    ...
    def CMF_PROG_INIT(self,CM_NMLIST,CC_NMLIST,log_filename,device,Datatype):
        ...
        def STORAGE_SEA_SURFACE(CM_NMLIST, Datatype, device):
            @torch.jit.script
            def compute_river_sequence(I1NEXT, D2RIVELV, D2RIVHGT, D2RIVLEN, D2RIVWTH, D2RIVSTOMAX,
                                       D2RIVDPH_PRE_1d, DDPH_1d, P2RIVSTO_1d,
                                       start: int, end: int, zero_tensor):

                indices = torch.arange(start, end, -1, device=I1NEXT.device)

                for i in range(len(indices)):
                    IESQ = indices[i]
                    JSEQ = I1NEXT[IESQ]-1
                    DSEAELV = D2RIVELV[JSEQ] + D2RIVDPH_PRE_1d[JSEQ]
                    # 计算水深 (向量化操作)
                    water_depth = DSEAELV - D2RIVELV[IESQ]
                    water_depth = torch.maximum(water_depth, zero_tensor)
                    water_depth = torch.minimum(water_depth, D2RIVHGT[IESQ])
                    DDPH_1d[IESQ] = water_depth

                    # 计算蓄水量 (向量化操作)
                    storage = water_depth * D2RIVLEN[IESQ] * D2RIVWTH[IESQ]
                    P2RIVSTO_1d[IESQ] = torch.minimum(storage, D2RIVSTOMAX[IESQ])

                    # 更新状态
                    D2RIVDPH_PRE_1d[IESQ] = water_depth
                return D2RIVDPH_PRE_1d, DDPH_1d, P2RIVSTO_1d
            ...
            ID                      =           torch.arange(CM_NMLIST.NSEQRIV + 1, CM_NMLIST.NSEQALL + 1, dtype=torch.int32)   #   !! river mouth grid (NSEQALL, not NSEQRIV)
            ...
            #  Pre compress all two-dimensional tensors into one dimension (reducing indexing overhead)
            I1NEXT                   =          CM_NMLIST.I1NEXT.raw()
            D2RIVELV                 =          CM_NMLIST.D2RIVELV.raw().squeeze(1)
            D2RIVHGT                 =          CM_NMLIST.D2RIVHGT.raw().squeeze(1)
            D2RIVLEN                 =          CM_NMLIST.D2RIVLEN.raw().squeeze(1)
            D2RIVWTH                 =          CM_NMLIST.D2RIVWTH.raw().squeeze(1)
            D2RIVSTOMAX              =          CM_NMLIST.D2RIVSTOMAX.raw().squeeze(1)
            D2RIVDPH_PRE_1d          =          self.D2RIVDPH_PRE.raw().squeeze(1)
            DDPH_1d                  =          self.DDPH.raw().squeeze(1)
            P2RIVSTO_1d              =          self.P2RIVSTO.raw().squeeze(1)

            zero_tensor              =          torch.tensor(0, dtype=Datatype.JPRB, device=device)

            D2RIVDPH_PRE_1d, DDPH_1d, P2RIVSTO_1d = compute_river_sequence(
                I1NEXT, D2RIVELV, D2RIVHGT, D2RIVLEN, D2RIVWTH, D2RIVSTOMAX,
                D2RIVDPH_PRE_1d, DDPH_1d, P2RIVSTO_1d,
                CM_NMLIST.NSEQRIV, 0, zero_tensor
            )

            self.D2RIVDPH_PRE[:, 1] = D2RIVDPH_PRE_1d
            self.DDPH[:, 1] = DDPH_1d
            self.P2RIVSTO[:, 1] = P2RIVSTO_1d
