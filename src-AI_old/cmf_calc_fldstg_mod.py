...
def CMF_CALC_FLDSTG_DEF(CM_NMLIST,CC_NMLIST,CC_VARS,device, Datatype):
    ...
    #   The excess water is allocated to the floodplain
        for ISEQ in DD_P_M_ID:
            ...
            if  I> CC_NMLIST.NLFP-1:
                ...
                CC_VARS.D2FLDDPH[ISEQ,0] =        DDPH_fil [ISEQ].item() + DSTO_add  / DWTH_fil [ISEQ].item() / CM_NMLIST.D2RIVLEN[ISEQ,0].item()
            else:
                DSTO_add                 =       DSTOALL[ISEQ].item()   -   DSTO_fil[ISEQ].item()
                DWTH_add [ISEQ]           =      (-DWTH_fil [ISEQ].item()   +
                                                 np.sqrt(DWTH_fil [ISEQ].item()**2 +2 * DSTO_add  / CM_NMLIST.D2RIVLEN[ISEQ,0].item() / CM_NMLIST.D2FLDGRD[ISEQ, 0, I].item())
                                                 )
                CC_VARS.D2FLDDPH[ISEQ,0]=       DDPH_fil [ISEQ].item() + CM_NMLIST.D2FLDGRD[ISEQ, 0, I].item() * DWTH_add[ISEQ].item()
        ...
        CC_VARS.P2RIVSTO[DD_P_M_ID, 0]       =          (CM_NMLIST.D2RIVSTOMAX[DD_P_M_ID, 0] +
                                                        CM_NMLIST.D2RIVLEN[DD_P_M_ID, 0] * CM_NMLIST.D2RIVWTH[DD_P_M_ID, 0] * CC_VARS.D2FLDDPH[DD_P_M_ID, 0])

        CC_VARS.P2RIVSTO[DD_P_M_ID, 0]       =          torch.minimum(CC_VARS.P2RIVSTO[DD_P_M_ID, 0], PSTOALL[DD_P_M_ID])

        CC_VARS.D2RIVDPH[DD_P_M_ID, 0]       =          (CC_VARS.P2RIVSTO[DD_P_M_ID, 0] / CM_NMLIST.D2RIVLEN[DD_P_M_ID, 0]  / CM_NMLIST.D2RIVWTH[DD_P_M_ID, 0])

        CC_VARS.P2FLDSTO[DD_P_M_ID, 0]       =          PSTOALL [DD_P_M_ID] - CC_VARS.P2RIVSTO[DD_P_M_ID, 0]

        CC_VARS.P2FLDSTO[DD_P_M_ID, 0]       =          torch.maximum(CC_VARS.P2FLDSTO[DD_P_M_ID, 0], torch.tensor(0,dtype=Datatype.JPRD,device=device))
        #   Computation of floodplain area and its proportion
        CC_VARS.D2FLDFRC[DD_P_M_ID, 0]       =          ((-CM_NMLIST.D2RIVWTH[DD_P_M_ID, 0] + DWTH_fil[DD_P_M_ID] + DWTH_add[DD_P_M_ID]) /
    ...
    CC_VARS.D2RIVDPH[DD_N_M_ID, 0]      =       DSTOALL[DD_N_M_ID] / CM_NMLIST.D2RIVLEN[DD_N_M_ID, 0] / CM_NMLIST.D2RIVWTH[DD_N_M_ID, 0]
    CC_VARS.D2RIVDPH[DD_N_M_ID, 0]      =       torch.maximum(CC_VARS.D2RIVDPH[DD_N_M_ID, 0], torch.tensor(0,dtype=Datatype.JPRB,device=device))
    ...
    CC_VARS.P0GLBSTONEW2                    =           sum(((CC_VARS.P2RIVSTO[FD_Index,0].view(-1)) + (CC_VARS.P2FLDSTO[FD_Index,0].view(-1))).tolist())
