...
class CMF_MAPS_NMLIST_MOD:
    ...
    def CMF_MAPS_NMLISTT(self, config):
        """
        ...
        with open(log_filename, 'a') as log_file:
            ...
            log_file.write("\n!---------------------!\n")
            log_file.write(f"CMF::TIME_NMLIST: namelist OPEN in unit:   {self.CSETFILE}\n")
            log_file.flush()
            log_file.close()
        ...
        with open(log_filename, 'a') as log_file:
            ...
            log_file.close()
        # --------------------------------------------------------------------------------------------------------------
    def CMF_RIVMAP_INIT(self,    Datatype,         config):
        # --------------------------------------------------------------------------------------------------------------
        """
        ...
        # --------------------------------------------------------------------------------------------------------------
        # --------------------------------------------------------------------------------------------------------------
        def READ_MAP_BIN(log_filename, Datatype, device):
            #   !*** read river map
            with open(log_filename, 'a') as log_file:
            ...
            with (open(self.CNEXTXY, 'rb') as f):
                I2NEXTX_temp            =           torch.frombuffer(bytearray(f.read(4 * self.NX * self.NY)),
                                                               dtype=self.I2NEXTX_TyPe).clone().reshape(self.NY, self.NX).T
                self.I2NEXTX[:self.NX+1, :self.NY+1]          =           I2NEXTX_temp
                I2NEXTY_temp            =           torch.frombuffer(bytearray(f.read(4 * self.NX * self.NY)),
                                                                dtype=self.I2NEXTY_TyPe).clone().reshape(self.NY, self.NX).T
                self.I2NEXTY[:self.NX + 1, :self.NY + 1]      =           I2NEXTY_temp
                f.close()
            ...
            if self.WEST >= -180.0 and self.EAST <= 360.0 and self.SOUTH >= -180.0 and self.NORTH <= 180.0:
                self.IX                   =          torch.arange(1, self.NX + 1, dtype=Datatype.JPIM, device=device)
                self.D1LON[:]             =          self.WEST + (self.IX - 0.5) * (self.EAST - self.WEST) / self.NX
                self.IY                   =          torch.arange(1, self.NY + 1, dtype=Datatype.JPIM, device=device)
                self.D1LAT[:]             =          self.NORTH - (self.IY - 0.5) * (self.NORTH - self.SOUTH) / self.NY
        # --------------------------------------------------------------------------------------------------------------
        # --------------------------------------------------------------------------------------------------------------
        def CALC_REGION(log_filename, device):
            ...
            mask                            =       Ftensor_2D(self.I2NEXTX.raw() != self.IMIS, start_row=1, start_col=1)
            ...
            with open(log_filename, 'a') as log_file:
                ...
                log_file.flush()
                log_file.close()
                self.REGIONGRID             =       torch.zeros ((self.REGIONALL), dtype=self.I2REGION.raw().dtype, device=device)
                ...
                with open(log_filename, 'a') as log_file:
                    log_file.write(f"CALC_REGION: REGIONALL=      {self.REGIONALL}\n")
        ...
        # --------------------------------------------------------------------------------------------------------------
        # --------------------------------------------------------------------------------------------------------------
        def CALC_1D_SEQ(Datatype, device):
            with open(log_filename, 'a') as log_file:
                log_file.write("RIVMAP_INIT: convert 2D map to 1D sequence\n")
                log_file.flush()
                log_file.close()

            NUPST_temp              =       torch.zeros((self.NX, self.NY), dtype=Datatype.JPIM,device=device)
            self.NUPST              =       Ftensor_2D(NUPST_temp, start_row=1, start_col=1)
            UPNOW_temp              =       torch.zeros((self.NX, self.NY), dtype=Datatype.JPIM,device=device)
            ...
            I2VECTOR_temp           =       torch.zeros((self.NX, self.NY), dtype=Datatype.JPIM,device=device)
        ...
        # I2REGION == REGIONTHIS	                  :     current model region
            mask                    =       Ftensor_2D((self.I2NEXTX.raw() < 0) & (self.I2NEXTX.raw() != self.IMIS) &
        ...
        if self.LMAPCDF:
            ...
            READ_MAP_BIN    (log_filename,   Datatype,  self.device)
        ...
        with open(log_filename, 'a') as log_file:
            ...
            log_file.flush()
            log_file.close()
            CALC_REGION      (log_filename,   self.device)
        ...
        #   !*** 3. conversion 2D map -> 1D vector
            ...
            CALC_1D_SEQ    (Datatype,  self.device)
        ...
        return
        # --------------------------------------------------------------------------------------------------------------
    def CMF_TOPO_INIT(self, Datatype,     CU,      config):
        """
        ...
        # --------------------------------------------------------------------------------------------------------------
        # --------------------------------------------------------------------------------------------------------------
        def READ_TOPO_BIN(log_filename,device,Datatype,CMF_UTILS_Class):
            import numpy as np

            R2TEMP_TyPe             =           Datatype.JPRM
        # --------------------------------------------------------------------------------------------------------------
            R2TEMP                  =           torch.zeros((self.NX, self.NY), dtype=R2TEMP_TyPe, device=device)
        ...
        #            accumulation in hydrological modeling.
            ...
            with open(self.CGRAREA, 'rb') as f:
                buffer              =           f.read(4 * self.NX * self.NY)
                data_np             =           np.frombuffer(buffer, dtype=np.float32).reshape(self.NY, self.NX).T
        ...
        # --------------------------------------------------------------------------------------------------------------
        # --------------------------------------------------------------------------------------------------------------
        def SET_FLDSTG(device,Datatype):
            """
        ...
        #     - DHGTPRE	         |     of the current layer         |      previous floodplain elevation layers.
            """
            self.D2FLDSTOMAX                =       torch.zeros((self.NSEQALL,1,self.NLFP),dtype=Datatype.JPRB, device=device)
            self.D2FLDSTOMAX                =       Ftensor_3D(self.D2FLDSTOMAX, start_depth=1, start_row=1, start_col=1)
            self.D2FLDGRD                   =       torch.zeros((self.NSEQALL,1,self.NLFP),dtype=Datatype.JPRB, device=device)
            self.D2FLDGRD                   =       Ftensor_3D(self.D2FLDGRD, start_depth=1, start_row=1, start_col=1)
            self.DFRCINC                    =       torch.tensor(self.NLFP, dtype=torch.float64,device=device).pow(-1)
            #!
        ...
        self.D2FLDHGT        =                  torch.zeros((self.NSEQMAX, 1, self.NLFP), dtype=self.D2FLDHGT_TyPe, device=self.device)
        ...
        with open(log_filename, 'a') as log_file:
            ...
            log_file.close()
        if not self.LMAPCDF:
            READ_TOPO_BIN       (log_filename,self.device,Datatype,CU)
        else:
    ...
    #   ! *** 3a. Calc Channel Parameters
        ...
        self.D2FLDSTOMAX                =       torch.zeros((self.NSEQMAX, 1, self.NLFP), dtype=self.D2FLDSTOMAX_TyPe, device=self.device)
        self.D2FLDSTOMAX                =       Ftensor_3D(self.D2FLDSTOMAX, start_depth=1, start_row=1, start_col=1)
        self.D2FLDGRD                   =       torch.zeros((self.NSEQMAX, 1, self.NLFP), dtype=self.D2FLDGRD_TyPe, device=self.device)
        self.D2FLDGRD                   =       Ftensor_3D(self.D2FLDGRD, start_depth=1, start_row=1, start_col=1)
        SET_FLDSTG          (self.device,Datatype)
