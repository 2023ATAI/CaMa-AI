...
class CMF_UTILS_MOD:
    ...
    def vecD2mapR(self,D2VAR,I1SEQX,I1SEQY,NSEQMAX,device):
        ...
        IX = I1SEQX.raw().long()
        IY = I1SEQY.raw().long()
        R2TEMP[IX, IY] = D2VAR[:NSEQMAX+1, 1].to(dtype=self.JPRM)
        return R2TEMP

    def vecP2mapP(self,P2VEC,I1SEQX,I1SEQY,NSEQMAX,device):
        import torch

        P2MAP = torch.zeros((self.NX, self.NY), dtype=P2VEC.raw().dtype, device=device)
        P2MAP = Ftensor_2D(P2MAP, start_row=1, start_col=1)

        # ------------------------------------------------------------------------------------------------------------------
        IX = I1SEQX.raw().long()
        IY = I1SEQY.raw().long()
        P2MAP[IX, IY] = P2VEC[:NSEQMAX+1, 1]
        return P2MAP
