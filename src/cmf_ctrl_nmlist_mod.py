...
class CMF_CTRL_NMLIST_MOD:
    def __init__(self ,config,    Datatype):
        ...
        # Formal CaMa-PyTorch runtime: exactly two execution backends.
        # The YAML ``device`` field is the only execution selector:
        #   device=cpu      -> optimized CPU backend
        #   device=cuda[:N] -> CUDA/GPU backend
        self.EXECUTION_BACKEND = torch.device(self.device).type
        if self.EXECUTION_BACKEND not in {"cpu", "cuda"}:
            raise ValueError(
                f"Unsupported CaMa-PyTorch device {self.device!r}. "
                "Use 'cpu' or 'cuda[:N]'."
            )
        ...
        self.LLEVEE         =       config["LLEVEE"]   if "LLEVEE"   in config  else False             # unsupported in CaMa-PyTorch v1.0
        self.LSEDOUT        =       config["LSEDOUT"]  if "LSEDOUT"  in config  else False             # unsupported in CaMa-PyTorch v1.0
        self.LTRACE         =       config["LTRACE"]   if "LTRACE"   in config  else False             # unsupported in CaMa-PyTorch v1.0
        ...
        ##============================
        #   #*** 1h. Output Settings
        self.IFRQ_OUT       =       config["IFRQ_OUT"] if "IFRQ_OUT" in config else 24  # output frequency: [1,2,3,...,24] hour
    ...
    def log_settings(self, config):
        ...
        with open(log_filename, 'a') as log_file:
            ...
            log_file.write("\n=== NAMELIST, NRUNVER ===\n")
            log_file.write(f"EXECUTION_BACKEND                         {self.EXECUTION_BACKEND}\n")
