...
def _zero_outflw_diagnostic(field):
    """Reset an outflw diagnostic tensor without attaching autograd state."""
    with torch.no_grad():
        field.raw().zero_()


def _accumulate_outflw_diagnostic(accumulator, current, dt):
    """Fortran-equivalent online time integral: accumulator += current * dt."""
    with torch.no_grad():
        accumulator.raw().add_(current.raw().detach(), alpha=float(dt))


def _finalize_outflw_diagnostic(accumulator, elapsed_time):
    """Convert the online time integral to a mean in place."""
    if float(elapsed_time) <= 0.0:
        raise ValueError("outflw diagnostic elapsed time must be positive")
    with torch.no_grad():
        accumulator.raw().div_(float(elapsed_time))
...
def CMF_DIAG_RESET_ADPSTP(log_filename, CC_VARS, CT_NMLIST, CC_NMLIST, CM_NMLIST, device, Datatype):
    ...
    _zero_outflw_diagnostic(CC_VARS.D2OUTFLW_aAVG)
    ...
    if CC_NMLIST.LDAMOUT:
        raise NotImplementedError("LDAMOUT diagnostic averages are not supported in CaMa-PyTorch v1.0.")
    if CC_NMLIST.LWEVAP :
        raise NotImplementedError("LWEVAP diagnostic averages are not supported in CaMa-PyTorch v1.0.")
...
def CMF_DIAG_AVEMAX_ADPSTP(CC_NMLIST,CC_VARS,CM_NMLIST,device):
    ...
    _accumulate_outflw_diagnostic(CC_VARS.D2OUTFLW_aAVG, CC_VARS.D2OUTFLW, CC_NMLIST.DT)
    ...
    if CC_NMLIST.LWEVAP:
        raise NotImplementedError("LWEVAP adaptive diagnostic accumulation is not supported in CaMa-PyTorch v1.0.")

    #   !! loop for optional variable (separated for computational efficiency)
    if CC_NMLIST.LDAMOUT:
        raise NotImplementedError("LDAMOUT adaptive diagnostic accumulation is not supported in CaMa-PyTorch v1.0.")

    if CC_NMLIST.LPTHOUT:
        NO_Index = torch.arange(
            1,
            CM_NMLIST.NPTHOUT + 1,
            device=device,
        )
        layer_slice = slice(1, CM_NMLIST.NPTHLEV + 1)
        CC_VARS.D1PTHFLW_aAVG[NO_Index, layer_slice] = (
            CC_VARS.D1PTHFLW_aAVG[NO_Index, layer_slice]
            + CC_VARS.D1PTHFLW[NO_Index, layer_slice] * CC_NMLIST.DT
        )

    if CC_NMLIST.LSEDOUT:
        raise NotImplementedError("LSEDOUT sediment diagnostics are not supported in CaMa-PyTorch v1.0.")
    return CC_VARS
# ----------------------------------------------------------------------------------------------------------------------
# ----------------------------------------------------------------------------------------------------------------------
def CMF_DIAG_GETAVE_ADPSTP(log_filename, CC_VARS, CT_NMLIST, CC_NMLIST, device, Datatype):
    ...
    _finalize_outflw_diagnostic(CC_VARS.D2OUTFLW_aAVG, CC_VARS.NADD_adp)
    ...
    if CC_NMLIST.LDAMOUT:
        raise NotImplementedError("LDAMOUT finalized adaptive diagnostics are not supported in CaMa-PyTorch v1.0.")
    if CC_NMLIST.LWEVAP:
        raise NotImplementedError("LWEVAP finalized adaptive diagnostics are not supported in CaMa-PyTorch v1.0.")
...
def CMF_DIAG_RESET_OUTPUT(log_filename, CC_VARS, CT_NMLIST, CC_NMLIST, CM_NMLIST, device, Datatype):
    ...
    _zero_outflw_diagnostic(CC_VARS.D2OUTFLW_oAVG)
    ...
    if CC_NMLIST.LDAMOUT:
        raise NotImplementedError("LDAMOUT output diagnostics are not supported in CaMa-PyTorch v1.0.")
    if CC_NMLIST.LWEVAP :
        raise NotImplementedError("LWEVAP output diagnostics are not supported in CaMa-PyTorch v1.0.")
...
def CMF_DIAG_AVEMAX_OUTPUT (CC_NMLIST,CC_VARS,CM_NMLIST,device):
    ...
    _accumulate_outflw_diagnostic(CC_VARS.D2OUTFLW_oAVG, CC_VARS.D2OUTFLW_aAVG, CC_NMLIST.DT)
    ...
    if CC_NMLIST.LWEVAP:
        raise NotImplementedError("LWEVAP output diagnostic accumulation is not supported in CaMa-PyTorch v1.0.")

    #   !! loop for optional variable (separated for computational efficiency)
    if CC_NMLIST.LDAMOUT:
        raise NotImplementedError("LDAMOUT output diagnostic accumulation is not supported in CaMa-PyTorch v1.0.")
    ...
    if CC_NMLIST.LPTHOUT:
        ...
        layer_slice = slice(1, CM_NMLIST.NPTHLEV + 1)
        CC_VARS.D1PTHFLW_oAVG[NO_Index, layer_slice] = (
            CC_VARS.D1PTHFLW_oAVG[NO_Index, layer_slice]
            + CC_VARS.D1PTHFLW_aAVG[NO_Index, layer_slice] * CC_NMLIST.DT
        )
...
def CMF_DIAG_GETAVE_OUTPUT(log_filename, CC_VARS, CT_NMLIST, CC_NMLIST, device, Datatype):
    ...
    _finalize_outflw_diagnostic(CC_VARS.D2OUTFLW_oAVG, CC_VARS.NADD_out)
    ...
    if CC_NMLIST.LDAMOUT:
        raise NotImplementedError("LDAMOUT finalized output diagnostics are not supported in CaMa-PyTorch v1.0.")
    if CC_NMLIST.LWEVAP:
        raise NotImplementedError("LWEVAP finalized output diagnostics are not supported in CaMa-PyTorch v1.0.")
