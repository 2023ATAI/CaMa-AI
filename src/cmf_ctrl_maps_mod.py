...
class CMF_MAPS_NMLIST_MOD:
    ...
    def CMF_RIVMAP_INIT(self, CC_NMLIST,         log_filename,            Datatype,         config):
        ...
        def READ_MAP_BIN(CCNMLIST_Class, log_filename, Datatype, device):
            ...
            if CCNMLIST_Class.LMAPEND:
                raise NotImplementedError("Endian conversion for map files is not supported in CaMa-PyTorch v1.0.")
        ...
        def CALC_REGION(CCNMLIST_Class, log_filename, device):
            ...
            I2REGION_temp                   =       torch.full(
                                                    (CCNMLIST_Class.NX, CCNMLIST_Class.NY), CCNMLIST_Class.IMIS,device=device)
        ...
        with open(log_filename, 'a') as log_file:
            ...
            log_file.close()
        if self.LMAPCDF:
            raise NotImplementedError("NetCDF river map input is not supported in CaMa-PyTorch v1.0.")
        else:
    ...
    def CMF_TOPO_INIT(self, CC_NMLIST,       log_filename,      Datatype,         CU):
        ...
        #            used to estimate flow resistance and velocity in hydraulic modeling.
            ...
            if CC_NMLIST.LGDWDLY:
                with open(log_filename, 'a') as log_file:
                    ...
                    log_file.flush()
                    log_file.close()
                raise NotImplementedError("LGDWDLY groundwater delay input is not supported in CaMa-PyTorch v1.0.")
            ...
            if CC_NMLIST.LMEANSL:
                with open(log_filename, 'a') as log_file:
                    ...
                    log_file.flush()
                    log_file.close()
                raise NotImplementedError("LMEANSL mean sea level input is not supported in CaMa-PyTorch v1.0.")
        ...
        if not self.LMAPCDF:
            ...
            raise NotImplementedError("NetCDF topography input is not supported in CaMa-PyTorch v1.0.")
