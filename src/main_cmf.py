...
# ----------------------------------------------------------------------------------------------------------------------
    backend = torch.device(config["device"]).type
    if backend not in {"cpu", "cuda"}:
        raise ValueError(
            f"Unsupported CaMa-PyTorch device {config['device']!r}. Use 'cpu' or 'cuda[:N]'."
        )
    if backend == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("CUDA backend requested, but torch.cuda.is_available() is False.")
    print(f"[CaMa-PyTorch] execution backend: {backend.upper()} ({config['device']})", flush=True)
    Datatype = Parkind1()
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    nsteps              =           _to_int(CT_NMLIST.NSTEPS)
    ...
    for ISTEP in range(1, nsteps + 1, ISTEPADV):
       ...
       # !*  2b Interporlate runoff & send to CaMa-Flood
        ...
        cmf_drv_advance_mod.ADVANCE(ISTEPADV,     CC_NMLIST ,     CM_NMLIST,    CT_NMLIST,
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    CMF_DRV_END     (config,CC_NMLIST,CF_NMLIST,CM_NMLIST,CO_NMLIST,CT_NMLIST)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, required=True, help='Path to the CaMa-PyTorch YAML configuration file.')
    parser.add_argument('--device', type=str, default=None, help='Override config device')
    parser.add_argument('--exp', type=str, default=None, help='Override config EXP')
    ...
    if args.exp is not None:
        config["EXP"] = args.exp.rstrip("/\\") + "/"

    config = run_config_1(config)
    while config["IYR"] <= config["YEND"]:
        config = run_config_2(config)
        config = main_cmAI(config)
        config = run_config_3(config)
# ----------------------------------------------------------------------------------------------------------------------
