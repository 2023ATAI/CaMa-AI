...
def _netcdf_dataset(*args, **kwargs):
    try:
        ...
        raise RuntimeError("NetCDF output requires the netCDF4 Python package.") from exc
...
class CMF_OUTPUT_NMLIST_MOD:
    def __init__(self, config, Datatype):
        ...
        self.COUTTAG                    =           "_cmf"                          #    Output tag name for each run
        #!
    ...
    def CMF_OUTPUT_INIT(self, CC_NMLIST,          log_filename,            CM_NMLIST,           CT_NMLIST,      config):
        ...
        #   ! Create file
            NCFILE                       =   _netcdf_dataset(self.VAROUT[JF].CFILE, 'w', format='NETCDF4')
            #   !=== set dimension ===
    ...
    def CMF_OUTPUT_WRITE(self, log_filename, CC_VARS, CT_NMLIST, CC_NMLIST, Datatype, device,CM_NMLIST,CU):
        ...
        def WRTE_OUTCDF(CT_NMLIST, R2OUT, JF):
            ...
            NCFILE = _netcdf_dataset(self.VAROUT[JF].CFILE, 'a', format='NETCDF4')
        ...
        #!*** 2. check variable name & allocate data to pointer DVEC
            for JF in range(self.NVARSOUT):
                ...
                if CVNAME ==        'rivsto':
                    ...
                    D2VEC       =                   CC_VARS.D2RIVOUT_oAVG
                elif CVNAME ==      'rivdph':
                    D2VEC       =                   CC_VARS.D2RIVDPH
                elif CVNAME ==      'rivvel':
                    D2VEC       =                   CC_VARS.D2RIVVEL_aAVG
                elif CVNAME ==      'fldout':
                    D2VEC       =                   CC_VARS.D2FLDOUT_oAVG
                    ...
                    D2VEC       =                   CC_VARS.D2OUTFLW_oAVG        #   !! output-window mean, matching Fortran D2OUTFLW_oAVG
                    ...
                    if not CC_NMLIST.LPTHOUT:
                        continue
                    D2VEC       =                   CC_VARS.D2PTHOUT_oAVG
                elif CVNAME ==      'pthflw':
                    if not CC_NMLIST.LPTHOUT:
                        continue
                elif CVNAME ==      'maxflw':
                    D2VEC       =                   CC_VARS.D2OUTFLW_oMAX
                elif CVNAME ==      'maxdph':
                    D2VEC       =                   CC_VARS.D2RIVDPH_oMAX
                elif CVNAME ==      'maxsto':
                    D2VEC       =                   CC_VARS.D2STORGE_oMAX
                ...
                if self.VAROUT[JF].CVNAME               !=      'pthflw':
                    ...
                    if not CC_NMLIST.LPTHOUT:
                        continue
                    R1POUT[:, :]     =                  CC_VARS.D1PTHFLW_oAVG[:, :].to(dtype=torch.float64)
