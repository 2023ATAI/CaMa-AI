...
class CMF_CTRL_VARS_MOD:
    ...
    * CONTAINS:
    ...
    def CMF_DIAG_INIT(self,CM_NMLIST,CC_NMLIST,log_filename,device):
        ...
        self.IDX_ALL                    =           torch.arange(1, CM_NMLIST.NSEQALL + 1, dtype=torch.long, device=device)
        self.IDX_RIVER                  =           torch.arange(1, CM_NMLIST.NSEQRIV + 1, dtype=torch.long, device=device)
        self.IDX_MOUTH                  =           torch.arange(CM_NMLIST.NSEQRIV + 1, CM_NMLIST.NSEQALL + 1, dtype=torch.long, device=device)
        self.ZERO_JPRB                  =           torch.zeros((), dtype=self.D2DAMMY_TyPe, device=device)
        self.ONE_JPRB                   =           torch.ones((), dtype=self.D2DAMMY_TyPe, device=device)
        self.EPS_JPRB                   =           torch.tensor(1.0e-10, dtype=self.D2DAMMY_TyPe, device=device)
        self.DEPTH_MIN_JPRB             =           torch.tensor(1.0e-6, dtype=self.D2DAMMY_TyPe, device=device)
