...
def _netcdf_dataset(*args, **kwargs):
    try:
        ...
        raise RuntimeError("NetCDF restart I/O requires the netCDF4 Python package.") from exc
...
class CMF_RESTART_NMLIST_MOD:
    ...
    def CMF_RESTART_INIT(self, config, CC_NMLIST, CC_VARS, device, CM_NMLIST, Datatype, CU):
        ...
        def READ_REST_CDF(config, CC_NMLIST, CC_VARS, device, CM_NMLIST, Datatype, CU):
            ...
            NCFILE                          =           _netcdf_dataset(self.CFILE, mode='r')
    ...
    def CMF_RESTART_WRITE(self,log_filename, CT_NMLIST,CC_NMLIST, CM_NMLIST, CC_VARS, Datatype, CU, device, config):
        ...
        def WRTE_BIN_MAP(P2VAR,TNAM,IREC,CM_NMLIST):
            ...
            #   !! Single Precision Restart
                ...
                raise NotImplementedError("Single-precision binary restart writing is not supported in CaMa-PyTorch v1.0.")
        # --------------------------------------------------------------------------------------------------------------
        # --------------------------------------------------------------------------------------------------------------
        ...
        def WRTE_REST_CDF(log_filename, CT_NMLIST, CC_NMLIST, CM_NMLIST, CC_VARS, Datatype, CU, device, config):
            ...
            if CM_NMLIST.REGIONTHIS == 1:
                NCID            =       _netcdf_dataset(self.CFILE, 'w', format='NETCDF4')
                ...
                if CC_NMLIST.LGDWDLY:
                    raise NotImplementedError("LGDWDLY restart variables are not supported in CaMa-PyTorch v1.0.")

                if CC_NMLIST.LDAMOUT:       #   !!! added
                    raise NotImplementedError("LDAMOUT restart variables are not supported in CaMa-PyTorch v1.0.")

                if CC_NMLIST.LLEVEE:        #   !!! added
                    raise NotImplementedError("LLEVEE restart variables are not supported in CaMa-PyTorch v1.0.")
            ...
            if CC_NMLIST.LPTHOUT and not CC_NMLIST.LSTOONLY:
                ...
                P1PTH[:,:]      =           CC_VARS.D1PTHFLW_PRE.raw()
                if CM_NMLIST.REGIONTHIS == 1:
                    try:
